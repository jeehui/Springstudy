...
public class HomeController {
	...
	// 일반적으로 index.jsp를 열어 주는 일을 한다. (welcome file 작업)
	
	// 반환타입 : string(열어 줄 View 이름 = JSP 이름)
	// 메소드명	: index(아무 일도 안하니까 맘대로 지으면 됨)
	// 매개변수 : 없음 (요청과 응답을 처리하는 request, response 선언)
	public String index(HttpServletRequest request) {
		
		//첫 미션. "hello" 텍 스트를 index.jsp로 보내보자.
		request.setAttribute("res", "hello"); 
		//res 이름을 아무렇게 적어도 됨, hello 값
		//res를 index.jsp로 가서 EL 즉, ${res}로 쓰면된다.
		...
		// index.jsp 어떻게 갈까요?
		// forward로 이동합니다. 원래 그래요. forward로 이동한다는 건 request와 함께 이동
	}
