...
public class OpenapiServiceImpl implements OpenapiService {
	...
	public String dailyBoxOffice(String targetDt){
		...
		//인증키
		String key = "a9339e91cdf3436f1af3e77716623388";
		...
		//API URL Connection
			...
			try(BufferedReader br = new BufferedReader(new InputStreamReader(con.getInputStream()))){
				...
				while((line = br.readLine()) != null) {
					sb.append(line); //append(function): 요소 끝에 삽입 할 HTML 문자열, DOM요소, jQuery객체 반환하는 함수
				}
			} catch(IOException e) {
				e.printStackTrace(); //응답 실패
			}
			// JSON 문자열 그대로 반환
			/* {
				 "boxOfficeResult":{
			...
			*/
			return sb.toString(); //문자열 반환하려면 toStrign하기!
